...
def generate_token(schema, pattern_identifier, pattern_break):
	...
	for pattern_group in pattern_groups:
		if pattern_group == "-":
			...
			token_glyphs.extend(
				iter(generate_token(schema, pattern_group[1:], pattern_break))
			)
	...
	return "".join (token_glyphs)

def generate_tokens(schema, pattern_identifier, pattern_break, count):
	return [
		generate_token(schema, pattern_identifier, pattern_break)
...
def generate_break(schema, pattern_identifier, pattern_break):
	pattern_break = schema["breaks"].get(f"{pattern_identifier}*", pattern_break)
...
def display_patterns(stream):
	...
	for identifier in identifiers:
		...
		if trim_pattern and len (pattern) > trim_pattern_length:
			pattern = f"{pattern[:(trim_pattern_length - 4)]} ..."
...
def display_glyphs(stream):
	...
	for identifier in identifiers:
		...
		if trim_example and len (example) > trim_example_length:
			example = f"{example[:(trim_example_length - 4)]} ..."
...
if __name__ == "__main__":
	...
	if command == "generate" and len (arguments) <= 4:
		...
		if pattern_break == 't':
			pattern_break = True
		elif pattern_break == 'f' or pattern_break != 's' and pattern_break == '':
			pattern_break = False
